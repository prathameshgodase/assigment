...
public class TestEmployee {
	public static void main(String[] args) throws Exception {
		...
		ApplicationContext ctxobj = new ClassPathXmlApplicationContext("Que12.xml");
		...
		List<Employee> avgsalEmployees = emplist.stream().filter(e -> e.esalary <= avgSalary).collect(Collectors.toList());
		...
		List<String> departmentsList = emplist.stream().map(e -> e.dept.deptname).distinct().collect(Collectors.toList());
